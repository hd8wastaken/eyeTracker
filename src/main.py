...
class eye:
    ...
    def draw(self, canvas):
        squash = max(0.06, 1.0 - self.blink)
        axes = (self.r, max(2, int(self.r * squash)))

        cv2.ellipse(canvas, (int(self.cx), int(self.cy)), axes, 0, 0, 360,
        ...
        if self.blink < 0.8:
            pr = int(self.r * 0.42)
            py = self.cy + (self.py - self.cy) * squash
            cv2.circle(canvas, (int(self.px), int(py)), pr, (15, 15, 15), -1, cv2.LINE_AA)
...
def draw_ball(canvas, cx, cy, r, bob, color):
    by = cy + bob
    cv2.circle(canvas, (int(cx), int(by)), r, color, -1, cv2.LINE_AA)
    cv2.circle(canvas, (int(cx), int(by)), r, (25, 25, 25), 6, cv2.LINE_AA)

    hl_r = int(r * 0.22)
    hl_x = int(cx - r * 0.4)
    hl_y = int(by - r * 0.45)
    overlay = canvas.copy()
    cv2.circle(overlay, (hl_x, hl_y), hl_r, (255, 255, 255), -1, cv2.LINE_AA)
    cv2.addWeighted(overlay, 0.18, canvas, 0.82, 0, dst=canvas)


def draw_mouth(canvas, cx, cy, curve, width, thickness=7):
    ax = int(width)
    ay = int(abs(curve) * 40) + 10
    start, end = (200, 340) if curve >= 0 else (20, 160)
    cv2.ellipse(canvas, (int(cx), int(cy)), (ax, ay), 0, start, end,
                (25, 25, 25), thickness, cv2.LINE_AA)


def main():
    logr("starting ball bot", Fore.CYAN)
    ...
    win_w, win_h = 800, 800
    ball_cx, ball_cy, ball_r = 400, 400, 340
    ball_color = (80, 180, 250)

    er = 90
    left = eye(310, 340, er)
    right = eye(490, 340, er)
    max_travel = er - int(er * 0.42) - 8

    mouth_cx, mouth_cy = 400, 540
    mouth_width = 100
    mouth_curve = 0.4
    ...
    next_blink_at = time.time() + np.random.uniform(2.5, 6.0)
    blinking = False
    blink_start = 0.0

    start_time = time.time()

    cv2.namedWindow("ball bot", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("ball bot", win_w, win_h)
    ...
    while True:
        try:
            ...
            canvas = np.full((win_h, win_w, 3), (245, 245, 245), dtype=np.uint8)

            logr(f"{'-'*60}", Fore.MAGENTA)
            logr(f"faces={len(faces)}", Fore.CYAN)

            now = time.time()
            bob = math.sin(now * 1.3) * 6
            ...
            if len(faces) > 0:
                ...
                avg_box_area = total_w / len(faces)
                ...
                target_curve = 0.6 - dn * 1.1
                mouth_curve = mouth_curve + (target_curve - mouth_curve) * 0.15

                size_norm = min(1.0, avg_box_area / (fw * fh * 0.15))
                target_width = 70 + size_norm * 60
                mouth_width = mouth_width + (target_width - mouth_width) * 0.15

                logr(f"mouth target_curve={target_curve:.2f} curve={mouth_curve:.2f} "
                     f"width={mouth_width:.1f}", Fore.WHITE)
            else:
                mouth_curve = mouth_curve + (0.1 - mouth_curve) * 0.05
                logr("no face, holding last pupil position", Fore.RED)

            if not blinking and now >= next_blink_at:
                blinking = True
                blink_start = now
            if blinking:
                t = (now - blink_start) / 0.14
                if t >= 1.0:
                    blinking = False
                    left.blink = 0.0
                    right.blink = 0.0
                    next_blink_at = now + np.random.uniform(2.5, 6.0)
                else:
                    amount = 1.0 - abs(2 * t - 1.0)
                    left.blink = amount
                    right.blink = amount

            draw_ball(canvas, ball_cx, ball_cy, ball_r, bob, ball_color)
            left.cy = 340 + bob
            right.cy = 340 + bob
            left.draw(canvas)
            right.draw(canvas)
            draw_mouth(canvas, mouth_cx, mouth_cy + bob, mouth_curve, mouth_width)

            if show_prev:
                sm = cv2.resize(frame, (180, int(180 * fh / fw)))
                sh, sw = sm.shape[:2]
                canvas[10:10 + sh, 10:10 + sw] = sm
                cv2.rectangle(canvas, (10, 10), (10 + sw, 10 + sh), (0, 0, 0), 1)

            cv2.imshow("ball bot", canvas)
